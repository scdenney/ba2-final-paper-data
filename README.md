...
Curated 11-corpus menu for the BA2 Digital Korea final research paper. Pick one corpus and write a 2,500–6,000-word research paper. See the [Final Paper page](https://scdenney.github.io/ba2_digital-korea/final-paper/) on the course site for the brief, rubric, and submission details.
...
- **#3 Colonial magazines and #4 *Kaebyok* are Hanmun-mixed.** Use the **Hanja-aware preprocessing script** on the [Data & Scripts](https://scdenney.github.io/ba2_digital-korea/data/) page — `hanja_preprocessing_mac-users.py` / `hanja_preprocessing_windows-users.py` for Orange, or `hanja_preprocessing.R` for RStudio. It converts Chinese characters to their Hangul readings before Kiwi tokenization, and routes the mostly-Classical-Chinese (Hanmun) articles to character-level tokenization, tagging each article `korean` or `hanmun` so you can analyze the two registers separately. Note in your data and methods section that the KNU sentiment dictionary is contemporary, so historical valence may not match perfectly.
