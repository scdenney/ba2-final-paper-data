...
1. **Did Chosun Ilbo and Hankyoreh frame the 2018 summits in measurably different sentiment registers?** Filter to `year == 2018`, apply the KNU sentiment dictionary, then use a Box Plot grouped by `newspaper`. Hypothesis: Hankyoreh's coverage is more positive about the summit diplomacy, Chosun Ilbo's is more skeptical.
2. **What topics dominate each summit episode?** Fit an 8- to 10-topic LDA on the 451 articles, then chart topic prevalence by `summit_episode`. Expectation: 2000 articles emphasize Sunshine Policy and family reunions; 2007 articles introduce economic cooperation (Kaesong); 2018 articles foreground denuclearization, sanctions, and the US-DPRK Singapore process.
3. **Are partisan framing differences greater at certain summits than others?** Run topic modeling on each year separately (2007 + 2018 only — 2000 has too few articles), then compare the newspaper-prevalence distributions across years.
4. **Do Hankyoreh and Chosun Ilbo articles cluster together by topic even at the embedding level?** Embed all 396 2018 articles with KLUE BERT, run k-means with k=4-6, then check whether `newspaper` is concentrated in particular clusters.
