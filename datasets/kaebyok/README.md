...
**Preprocessing:** *Kaebyok* often mixes Hangul with Hanja (Chinese characters), and some articles are largely Classical Chinese (Hanmun). Use the **Hanja-aware preprocessing script** on the [Data & Scripts page](https://scdenney.github.io/ba2_digital-korea/data/) — `hanja_preprocessing_mac-users.py` / `hanja_preprocessing_windows-users.py` for Orange, or `hanja_preprocessing.R` for RStudio. It converts Hanja to Hangul readings before Kiwi tokenization, and routes the mostly-Classical-Chinese (Hanmun) articles to character-level tokenization, tagging each article `korean` or `hanmun` so you can analyze the two registers separately. Note that the KNU sentiment dictionary is contemporary, so historical valence may not match perfectly.
